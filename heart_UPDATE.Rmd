...
#plot the ROC curve
testroc4 = roc(predictor = as.data.frame(rf_random$finalModel$votes)$Yes, response = rf_random$finalModel$y, levels = rev(levels(rf_random$finalModel$y)))

plot(testroc4, type = "S")
text(x = .40, y = .6,paste("AUC = ", round(testroc4$auc, 3)))
<<<<<<< HEAD
...
>>>>>>> 030ca01fa3cfae33664981cb61a555ac18a56060
...
<<<<<<< HEAD
testroc3 <- roc(predictor = complex_model$pred$Yes, response = complex_model$pred$obs, levels = rev(levels(complex_model$pred$obs)),
=======
testroc3 <- roc(predictor = simple_model_stepwise$pred$Yes, response = simple_model_stepwise$pred$obs, levels = rev(levels(simple_model_stepwise$pred$obs)),
>>>>>>> 030ca01fa3cfae33664981cb61a555ac18a56060
              smoothed=FALSE, ci=FALSE, plot=TRUE, print.auc=TRUE)
...
#fit the simple model using all the variables
complex_model2 = train(target~ thal + ca + cp + sex + trestbps + exang + oldpeak + slope + thal*oldpeak + trestbps*exang, data = heart, trControl = train_control, method = "glmStepAIC", family = "binomial")

complex_model2$results

#plot the ROC curve
testroc3 <- roc(predictor = complex_model2$pred$Yes, response = complex_model2$pred$obs, levels = rev(levels(complex_model2$pred$obs)),
              smoothed=FALSE, ci=FALSE, plot=TRUE, print.auc=TRUE)
```
...
#You can run this if you just add a small value to oldpeak
